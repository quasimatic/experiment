...
describe("Locator: Contains Match", function () {
    ...
    it("should find by exact text match", function () {
        ...
        findContainsText({label:"contains text", container:document.body}).should.deep.equal([dom.get("target")]);
    });
    ...
    it("should find more than one", function () {
        ...
        findContainsText({label:"contains text", container:document.body}).should.deep.equal(dom.get("target-1", "target-2"));
    });

    it("should not find missing text", function () {
        dom.render(<div>contains text</div>)

        findContainsText({label:"missing text", container:document.body}).should.deep.equal([]);
    })
    ...
    it("should find inside a textnode", function () {
        ...
        findContainsText({label:"text node", container:document.body}).should.deep.equal([dom.get("target")]);
    })

    it("should not search script tags", function () {
        dom.render(<script>console.log('stuff')</script>)

        findContainsText({label:"stuff", container:document.body}).should.deep.equal([]);
    })
    ...
    it("should not search style tags", function () {
        ...
        findContainsText({label:"red", container:document.body}).should.deep.equal([]);
    })
});